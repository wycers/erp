...
原料（material_sku）表 SHALL 包含一个可选的 `unit` 字段，用于存储计量单位（如"米"、"个"、"件"等）。
...
成品（finished_product）表 SHALL 包含一个可选的 `unit` 字段，用于存储计量单位。
...
#### Scenario: Display material without unit
- **WHEN** 原料没有单位信息
- **THEN** 页面 SHALL 正常显示原料，单位位置显示为空或占位符
...
#### Scenario: Display product without unit
- **WHEN** 成品没有单位信息
- **THEN** 页面 SHALL 正常显示成品，单位位置显示为空或占位符

### Requirement: Unit input in forms
原料和成品的创建/编辑表单 SHALL 包含单位输入框。

#### Scenario: Material form unit input
- **WHEN** 用户在原料创建表单中
- **THEN** 表单 SHALL 显示单位输入框，允许用户输入自定义单位文本
...
- **WHEN** 用户查看库存列表
- **THEN** 每个库存项 SHALL 显示其关联的单位信息（如果有）
